...
void ObstacleWatcher(void *arg) {
  ...
  for (;;) {
    ...
    osMessageQueuePut(writer.args.qid, &readingRequest, MSG_PRIO,
                      osWaitForever);
    osStatus_t status = osMessageQueueGet(o->args.qid, &reading, MSG_PRIO,
                                          OBSTACLE_WATCHER_READ_TIMEOUT);
    if (status != osOK) {
      continue;
    }

    if (reading.sensor_reading < 0)
      continue;

    if (reading.sensor_reading <= OBSTACLE_WARNING_DISTANCE &&
        track.args.reference != TRACK_LEFT_REFERENCE) {
      track.args.reference = TRACK_LEFT_REFERENCE;
      osTimerStart(timer, SECOND * OBSTACLE_WARNING_DISTANCE /
                              speed_ctl.args.target_speed);
    }
  }
}